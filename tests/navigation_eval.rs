...
fn prompt(case: &str) -> &'static str {
    match case {
        ...
        "java" => {
            "src/sample/Service.java와 Parser.java를 구조 탐색으로 조사해줘. Service에 직접 선언된 load 오버로드와 Inner의 동명 메서드를 구분하고, String 인자 처리의 파일 간 호출, 정수 0과 음수 처리, Parser의 null 처리를 구현에서 확인해. 파일 수정 없이 JSON만 답해: direct_load_overloads(직접 선언된 오버로드 수), string_delegate(클래스.메서드), zero_accepted(boolean), negative_throws(boolean), nested_prefix(Inner.load가 붙이는 문자열), null_parse_result(Parser.parse(null)의 반환 문자열), citations({path,start,end} 배열, 프로젝트 상대 경로)."
        }
        _ => unreachable!(),
    }
}
...
fn semantic_oracle_rejects_missing_and_inverted_facts() {
    for case in ["semantics", "recovery", "java"] {
        let good = facts(case);
...
async fn live_navigation_regressions() {
    ...
    let cases = std::env::var("MNEMOARC_NAV_CASES").unwrap_or("semantics,recovery,java".into());
    for case in cases.split(',') {
        assert!(
            ["semantics", "recovery", "java"].contains(&case),
            "unknown navigation case"
        );
        for repetition in 1..=repetitions {
            let mut project = if case == "java" {
                java.clone()
            } else {
                registered.clone()
            };
