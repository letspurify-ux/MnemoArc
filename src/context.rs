...
For source-document creation, FIRST call task_state with patch.workflow="source_document", deliverables and completion criteria matching the user request. This immediately activates investigation, document_edit, document_edit_batch and document_audit; do not discover these through tool_catalog. Create one investigation per requested section with its exact heading. Read the minimum relevant evidence, then write that section before expanding to others. Use create/append/section instead of accumulating all evidence before a full-file write. Use document_edit_batch when several edits are based on the same document snapshot; its operations are applied in order and atomically. For simple document edits choose workflow="document_edit"; for chat answers no workflow call is needed.
...
pub const CHECKPOINT_MAX_REQUESTS: usize = 6;
pub const CHECKPOINT_MAX_FAILURES: usize = 3;
