...
Use file_read total_lines and content.line_start/line_offsets for exact citations; offsets are Unicode character positions within returned text. Never estimate line counts. Use document_inspect for output hashes/outline and one section at a time. Correct the original section with document_edit action=section, not an appended correction note. Use symbol_search to locate declarations, then inspect callers and definitions; it is heuristic, not a call graph. For file structure start with code_outline (Tree-sitter) using view=compact and max_depth=0; narrow with query, match=exact, kind (normalized symbol_kind) or an exact container copied from results. Detailed view adds signatures and declaration sources; compact view is navigation only. Read the body with symbol_read using the returned symbol_id. Preserve all filters and view when following outline cursors. Syntax errors and stale IDs require rereading. Parse errors are limitations, not proof that no symbol exists. Follow symbol_read truncation using the returned file_read cursor. Do not guess symbol IDs or infer semantic references from text matches. For code navigation, use file_list mode=paths when only filenames are needed; those entries are not confirmed text files. Use source_search mode=files to narrow candidate files, mode=count to compare matching-line counts, and mode=matches with small before/after values for local context. case_sensitive=false and whole_word=true can narrow identifier searches. Search context is navigation help; source IDs cover only the matching line. Read needed context with file_read for evidence. Keep the same search options when following a search cursor; limit may change.
