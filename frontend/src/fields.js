...
export const groups = [
  ...
  },
  {
    ...
    fields: [
      ...
      ["proxy", "프록시 주소", "optional", "비우면 환경변수·시스템 프록시가 적용될 수 있습니다."],
      ["disable_proxy", "프록시 강제 비활성화", "boolean", "LLM 요청에서 프록시 주소와 환경변수·시스템 프록시를 모두 무시하고 직접 연결합니다."],
      [
        "model_context",
        "모델 최대 컨텍스트",
        "optionalNumber",
  ...
  },
  {
    ...
    fields: [
      ...
      [
        ...
        "기억 목록 한도",
        "number",
        "기억 인덱스의 기본 예산은 최신 항목당 160 토큰 + 공통 128 토큰이며, 큰 항목은 예산에 맞춰 상태에서 생략되고 memory_find/read로 조회할 수 있습니다.",
      ],
  ...
  },
  {
    ...
    fields: [
      ...
      [
        ...
        "진행 정체 감지 횟수",
        "number",
        "문서 변경·검증 없이 모델 호출이 반복되면 작성에 집중합니다. 같은 도구 오류가 반복되면 별도로 중단합니다",
      ],
      [
        "document_repair_limit",
        "재검토 전 수정 요청 수",
        "number",
        "리뷰 지적 후 이 횟수만큼 편집을 시도하면 다음 편집 전에 현재 문서를 다시 검토합니다 (기본 8, 실패 포함). 읽기·조회·검증은 제외합니다.",
      ],
      ["review_limit", "진전 없는 문서 검토 한도", "number", "같은 문제를 해결하지 못한 연속 검토의 한도. 문제가 줄거나 해결되면 다시 검토를 이어갑니다."],
    ],
  },
];
