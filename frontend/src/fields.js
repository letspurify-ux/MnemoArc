...
export const groups = [
  ...
  },
  {
    ...
    fields: [
      ...
      [
        "closing_reserve_ratio",
        "마감 예산 비율",
        "percent",
        "이 비율의 예산이 남거나 진행이 오래 멈추면 탐색을 멈추고 문서를 마무리합니다. 확인하지 못한 항목은 결과에 명시합니다 (검증 비율보다 작게)",
      ],
      [
        "repeated_read_limit",
        "동일 범위 반복 조회 제한",
      ...
      [
        ...
        "문서 수정 후 재검토 간격",
        "number",
        "리뷰 지적 후 이 횟수만큼 편집을 시도하면 해당 요청을 모두 처리한 뒤 재검토합니다 (기본 8, 실패 포함). 바뀌지 않은 문서는 기존 지적을 재사용하고 수정을 계속합니다.",
      ],
